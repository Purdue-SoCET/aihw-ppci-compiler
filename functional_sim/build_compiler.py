...
CONTROL_MNEMONICS = {"jal", "jalr", "halt.s", "ret"}
...
def asm_to_instr_dict(
    ...
    *,
    ...
    if mnemonic in {"add.vv", "sub.vv", "mul.vv", "gemm.vv"} and len(ops) == 4:
...
def preserve_compiler_packets(
    instructions: list[AsmInstr],
) -> tuple[list[int], list[list[int]]]:
    """Preserve packetization already emitted by the compiler.

    This path intentionally does not schedule, reorder, or insert latency stalls.
    It groups the incoming assembly stream into 4-slot packets and trusts compiler
    emitted ``nop`` instructions to represent unused packet slots.
    """
    packets = [
        list(range(idx, min(idx + base.REAL_PACKET_SIZE, len(instructions))))
        for idx in range(0, len(instructions), base.REAL_PACKET_SIZE)
    ]
    issue_cycle = [0 for _ in range(len(instructions))]
    for pkt_idx, packet in enumerate(packets):
        for idx in packet:
            issue_cycle[idx] = pkt_idx
    return issue_cycle, packets
...
def relocate_and_encode(
    ...
    for packet in packets:
        for idx in packet:
            ...
            hex40 = base.encode_instruction(instr_dict).upper()
            if len(hex40) != 10:
...
def compile_asm(in_data: str) -> tuple[str, list[int], list[list[int]]]:
    """Encode compiler-oriented assembly into a packetized ``.in`` text image.
    ...
    Then: parse → optional main bootstrap → ``expand_large_li`` → preserve the
    compiler-emitted 4-slot packet stream → encode.
    """
    ...
    if "main" in labels:
        bootstrap = [
            AsmInstr(mnemonic="jal", ops=["$1", "main"], comment="bootstrap.call_main"),
            AsmInstr(mnemonic="nop.s", ops=[], comment=""),
            AsmInstr(mnemonic="nop.s", ops=[], comment=""),
            AsmInstr(mnemonic="nop.s", ops=[], comment=""),
            AsmInstr(mnemonic="halt.s", ops=[], comment="bootstrap.halt"),
            AsmInstr(mnemonic="nop.s", ops=[], comment=""),
        ...
        labels = {name: idx + len(bootstrap) for name, idx in labels.items()}
        instructions = bootstrap + instructions
    instructions, labels = expand_large_li(instructions, labels)
    # Packetization is intentionally disabled here so the simulator image uses
    # the packet stream produced by the branch compiler's `-p` path.
    # ready, packets = schedule_program(instructions, labels)
    ready, packets = preserve_compiler_packets(instructions)
