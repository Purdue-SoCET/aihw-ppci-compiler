...
def asm_to_instr_dict(
    ...
    *,
    ...
    # ISA update: rcp.bf is unary (rd = 1 / rs1).
    # Keep backward compatibility with legacy 3-operand form when rs2 == $0.
    if mnemonic == "rcp.bf":
        if len(ops) not in (2, 3):
            raise ValueError("rcp.bf expects 2 operands (rd, rs1)")

        rd = base.parse_reg(ops[0])
        rs1 = base.parse_reg(ops[1])

        if len(ops) == 3 and base.parse_reg(ops[2]) != 0:
            raise ValueError("rcp.bf legacy third operand must be $0")

        opcode, instr_type = base.INVERT_OPCODES["rcp.bf"]
        return {
            "opcode": opcode,
            "type": instr_type,
            "rd": rd,
...
def _is_reg_operand(op: str) -> bool:
    try:
        _parse_reg(op)
        return True
    except Exception:
...
def _build_sched_info(inst: AsmInstr, idx: int) -> SchedInfo:
    ...
    if instr_type == "R":
        ...
        if len(inst.ops) >= 3 and _is_reg_operand(inst.ops[2]):
            rs3 = _parse_reg(inst.ops[2])
            _add_reg(reads, _reg_scalar(rs3))

            packed_meta = None
            if len(inst.ops) in (6, 7):
                sid = _parse_imm(inst.ops[3])
                num_rows = _parse_imm(inst.ops[4])
                num_cols = _parse_imm(inst.ops[5])
                num_cols_full = _parse_imm(inst.ops[6]) if len(inst.ops) == 7 else num_cols
                packed_meta = base.pack_sdma_metadata(
                    sid=sid,
                    num_rows=num_rows,
                    num_cols=num_cols,
                ...
                raise ValueError(
                    f"{mnemonic} expects 3 operands (new SDMA form) or 6/7 operands (new+metadata form), got {len(inst.ops)}"
                )

            mem_key = ("scpad_mem_rs3", rs1_rd1, rs2, rs3, packed_meta)
        else:
            if len(inst.ops) not in (5, 6):
                raise ValueError(
                    f"{mnemonic} expects 5/6 operands (legacy SDMA form) or 3 operands (new SDMA form), got {len(inst.ops)}"
                )
            num_cols = _parse_imm(inst.ops[2])
            num_rows = _parse_imm(inst.ops[3])
            sid = _parse_imm(inst.ops[4])
            num_cols_full = _parse_imm(inst.ops[5]) if len(inst.ops) == 6 else num_cols
            packed_meta = base.pack_sdma_metadata(
                sid=sid,
                num_rows=num_rows,
                num_cols=num_cols,
