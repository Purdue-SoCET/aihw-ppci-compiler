...
def reg_bf_or_fp32_to_fp32(x):
    x_u32 = int(x) & 0xFFFFFFFF
    # ISA update compatibility:
    # - preferred form: register holds raw BF16 bits in low 16 bits
    # - legacy form: register holds full FP32 bit pattern
    if (x_u32 & 0xFFFF0000) == 0:
        return struct.unpack('<f', struct.pack('<I', (x_u32 & 0xFFFF) << 16))[0]
    return hex_to_fp32(x_u32)
    ...
    while (not(halt)):
        ...
        for inst in dec_packet:
            ...
            #scpad load/store here
                ...
                src1 = sregs.read(inst['rs1'])
                src1 = reg_bf_or_fp32_to_fp32(src1)
