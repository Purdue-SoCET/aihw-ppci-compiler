...
def pack_sdma_metadata(*, sid: int, num_rows: int, num_cols: int, num_cols_full: int) -> int:
    if not (0 <= sid <= 0x3):
        raise ValueError(f"SDMA sid out of range [0, 3]: {sid}")
    if not (0 <= num_rows <= 0x1F):
        raise ValueError(f"SDMA num_rows out of range [0, 31]: {num_rows}")
    if not (0 <= num_cols <= 0x1F):
        raise ValueError(f"SDMA num_cols out of range [0, 31]: {num_cols}")
    if not (0 <= num_cols_full <= 0xFFFFF):
        raise ValueError(f"SDMA num_cols_full out of range [0, 1048575]: {num_cols_full}")

    return (
        ((sid & 0x3) << 30)
        | ((num_rows & 0x1F) << 25)
        | ((num_cols & 0x1F) << 20)
        | (num_cols_full & 0xFFFFF)
    )
...
def encode_instruction(instr_dict):
    ...
    if instr_type == "R":
        ...
        # SDMA (new): rs1/rd1 7-14, rs2 15-22, rs3(metadata register) 23-30
        # SDMA (legacy): rs1/rd1 7-14, rs2 15-22, num_cols 23-27, num_rows 28-32, sid 33
        ...
        if 'rs3' in instr_dict:
            instruction |= (instr_dict.get('rs3', 0) & 0xFF) << 23
        else:
            num_cols = instr_dict.get('num_cols', 0)
            num_rows = instr_dict.get('num_rows', 0)
            sid = instr_dict.get('sid', 0)
...
def asm_to_instr_dict(
    ...
    *,
    ...
    if instr_type == "R":
        if mnemonic == "rcp.bf" and len(ops) == 2:
            # ISA semantics: rd = 1 / rs1.
            # Encoding is R-type, so rs2 is ignored and encoded as x0.
    ...
    if instr_type == "SDMA":
        # Supported forms:
        # - New:    scpad.ld rs1_rd1, rs2, rs3
        # - New+:   scpad.ld rs1_rd1, rs2, rs3, sid, num_rows, num_cols[, num_cols_full]
        # - Legacy: scpad.ld rs1_rd1, rs2, num_cols, num_rows, sid[, num_cols_full]
        d["rs1"] = parse_reg(ops[0])
        d["rs2"] = parse_reg(ops[1])

        if len(ops) >= 3 and REG_RE.match(ops[2].strip()):
            d["rs3"] = parse_reg(ops[2])
            if len(ops) in (6, 7):
                sid = parse_int(ops[3])
                num_rows = parse_int(ops[4])
                num_cols = parse_int(ops[5])
                num_cols_full = parse_int(ops[6]) if len(ops) == 7 else num_cols
                d["sid"] = sid
                d["num_rows"] = num_rows
                d["num_cols"] = num_cols
                d["num_cols_full"] = num_cols_full
                d["sdma_meta"] = pack_sdma_metadata(
                    sid=sid,
                    num_rows=num_rows,
                    num_cols=num_cols,
                ...
                raise ValueError(
                    f"{mnemonic} expects 3 operands (new form) or 6/7 operands (new+metadata form), got {len(ops)}"
                )
            return d

        if len(ops) not in (5, 6):
            raise ValueError(
                f"{mnemonic} expects 5/6 operands (legacy form) or 3 operands (new form), got {len(ops)}"
            )

        num_cols = parse_int(ops[2])
        num_rows = parse_int(ops[3])
        sid = parse_int(ops[4])
        num_cols_full = parse_int(ops[5]) if len(ops) == 6 else num_cols
...
def _decode_instruction_for_graph(hex_word: str):
    raw = int(hex_word, 16)
    opcode = raw & 0x7F
    if opcode not in OPCODES:
        return "nop.s", [], [], None
    ...
    if instr_type == "R":
        ...
        vd = (raw >> 7) & 0xFF
        rs1 = (raw >> 15) & 0xFF
        num_cols = (raw >> 23) & 0x1F
        num_rows = (raw >> 28) & 0x1F
        sid = (raw >> 33) & 0x1
        rc = (raw >> 34) & 0x1
        rc_id = (raw >> 35) & 0x1F
        ...
        rs1_rd1 = (raw >> 7) & 0xFF
        rs2 = (raw >> 15) & 0xFF
        legacy_num_cols = (raw >> 23) & 0x1F
        legacy_num_rows = (raw >> 28) & 0x1F
        legacy_sid = (raw >> 33) & 0x1
        legacy_upper = (raw >> 31) & 0x1FF
        if legacy_upper != 0:
            srcs = [r(rs1_rd1), r(rs2)]
            mem_key = ("scpad_mem_legacy", legacy_sid, rs1_rd1, rs2, legacy_num_cols, legacy_num_rows)
        else:
            rs3 = (raw >> 23) & 0xFF
            srcs = [r(rs1_rd1), r(rs2), r(rs3)]
            mem_key = ("scpad_mem_rs3", rs1_rd1, rs2, rs3)
...
def _op_latency(op: str, latency_map):
    op_norm = op.lower()
...
def build_dependency_graph(instructions, latency_map, single_lsu=True):
    ...
    for i, (op, dsts, srcs, mem_key) in enumerate(instructions):
        ...
        if is_mem and mem_key is not None and mem_key in last_store_at and last_store_at[mem_key] > start:
