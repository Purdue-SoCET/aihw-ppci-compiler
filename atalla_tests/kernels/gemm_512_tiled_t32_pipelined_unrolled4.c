...
/*
 * TILE=32 pipelined GEMM (unrolled4 variant) with configurable weight reuse
 * across M tiles. cfg[10] = reuse_m_tiles in [1,32].
 */
int main(void) {
    ...
    for (int ni = 0; ni < N_tiles; ni = ni + 1) {
        ...
        scpad_load(SP_BYTE_W, w_n_base_b, sdma_ctl_w);
        ...
        for (int ki = 0; ki < K_tiles; ki = ki + 1) {
            ...
            if (ki + 1 < K_tiles) {
                int w_next = w_n_base_b + (ki + 1) * w_k_stride_b;
                scpad_load(SP_BYTE_W, w_next, sdma_ctl_w);
                w_prefetched = 1;
            } else {
                w_prefetched = 0;
            }
