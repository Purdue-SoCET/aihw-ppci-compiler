...
int main() {
    ...
    while (mi < M_tiles) {
        ...
        while (ni < N_tiles) {
            ...
            asm("scpad_ld %0, %1, 3, 3, 1" : : "r"(sp_c), "r"(c_addr));
            ...
            while (ki < K_tiles) {
                ...
                /* load A and W tiles */
                asm("scpad_ld %0, %1, 3, 3, 0" : : "r"(sp_a), "r"(a_addr));
                asm("scpad_ld %0, %1, 3, 3, 0" : : "r"(sp_w), "r"(w_addr));

                /* preload W rows into systolic array */
                int wi = 0;
                while (wi < TILE) {
                    vec wvec;
                    asm("vreg_ld %0, %1, 3, 3, 0, 1, 0"
                        : "=v"(wvec) : "r"(wi));
                    wi = wi + 1;
                }
                ...
                while (ri < TILE) {
                    ...
                    asm("vreg_ld %0, %1, 3, 3, 0, 1, 0"
                        : "=v"(a_row) : "r"(ri));
                    asm("vreg_ld %0, %1, 3, 3, 1, 1, 0"
                        : "=v"(c_row) : "r"(ri));

                    vec result = gemm(a_row, c_row, all_mask);

                    asm("vreg_st %0, %1, 3, 3, 1, 1, 0"
                        : : "v"(result), "r"(ri));
                    ri = ri + 1;
                }
            ...
            asm("scpad_st %0, %1, 3, 3, 1" : : "r"(sp_c), "r"(c_addr));
            ni = ni + 1;
        }
        mi = mi + 1;
    }
