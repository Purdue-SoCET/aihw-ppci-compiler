...
int main() {
    ...
    asm("scpad_ld %0, %1, 31, 0, 0" : : "r"(sp), "r"(IN_GMEM));

    int row = 0;
    while (row < ROWS) {
        vec v;
        asm("vreg_ld %0, %1, 31, 0, 0, 1, 0" : "=v"(v) : "r"(row));
        ...
        asm("vreg_st %0, %1, 31, 0, 0, 1, 0" : : "v"(result), "r"(row));
        row = row + 1;
    }

    /* store result back in-place */
    asm("scpad_st %0, %1, 31, 0, 0" : : "r"(sp), "r"(IN_GMEM));
